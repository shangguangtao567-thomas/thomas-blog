...
function renderShell(html, route) {
  let output = html;
  output = setTitle(output, route.title);
  output = upsertMeta(output, 'name', 'description', route.description);
  output = upsertMeta(output, 'name', 'robots', route.robots || 'index,follow');
  output = upsertMeta(output, 'property', 'og:type', route.type || 'website');
  output = upsertMeta(output, 'property', 'og:site_name', siteConfig.siteName);
  output = upsertMeta(output, 'property', 'og:title', route.title);
  output = upsertMeta(output, 'property', 'og:description', route.description);
  output = upsertMeta(output, 'property', 'og:url', route.url);
  output = upsertMeta(output, 'name', 'twitter:card', route.image ? 'summary_large_image' : 'summary');
  output = upsertMeta(output, 'name', 'twitter:title', route.title);
  output = upsertMeta(output, 'name', 'twitter:description', route.description);
  output = upsertMeta(output, 'name', 'twitter:site', '@GuangtaoS29545');
  output = upsertMeta(output, 'name', 'twitter:creator', '@GuangtaoS29545');
  output = upsertLink(output, 'canonical', route.url);
  output = upsertLink(output, 'alternate', `${SITE_URL}/feed.xml`, 'type="application/rss+xml" title="RSS Feed"');

  if (route.keywords?.length) {
    output = upsertMeta(output, 'name', 'keywords', route.keywords.join(', '));
  }

  if (route.image) {
    output = upsertMeta(output, 'property', 'og:image', route.image);
    output = upsertMeta(output, 'name', 'twitter:image', route.image);
  }

  if (route.publishedAt) {
    output = upsertMeta(output, 'property', 'article:published_time', route.publishedAt);
  }

  if (route.jsonLd) {
    output = injectIntoHead(output, `<script type="application/ld+json">\n${route.jsonLd}\n  </script>`);
  }
...
function renderStaticHome({ posts, digests }) {
  ...
  return `
    <div class="site-container fade-in" style="padding-top: 4rem; padding-bottom: 6rem;">
      ...
      <section>
        <div style="display: flex; justify-content: space-between; align-items: baseline; margin-bottom: 1rem;"><p class="section-label">AI Briefing</p><a href="/blog" style="font-size: 0.75rem; color: #6b6b6b;">Archive →</a></div>
        <div>${digestItems}</div>
      </section>
    </div>
  `;
...
async function main() {
  ...
  // Briefing list page (redirect to /blog for AI digests)
  const briefingRoute = { ...baseRoute, title: `AI Briefing · ${siteConfig.siteName}`, url: routeUrl('blog') };
  const briefingHtml = `
    <div class="site-container fade-in" style="padding-top: 3rem; padding-bottom: 6rem;">
      <div style="margin-bottom: 2.5rem;">
  ...
  // AI Daily Digest pages (from ai-digests.json)
  for (const digest of digests) {
    const detail = digestDetails.find(d => d.slug === digest.slug);
    const digestRoute = {
      ...baseRoute,
      title: `${digest.titleEn} · Thomas`,
      description: truncateText(digest.excerptEn || digest.issueSummaryEn, 155),
      url: routeUrl(`blog/${digest.slug}`),
      type: 'article',
      publishedAt: digest.date,
      section: 'AI Daily',
    };

    // Build digest body from detail content
    const digestBodyHtml = detail?.items?.length
      ? detail.items.map(item => `
          <div style="margin-bottom: 2rem;">
            <h3 style="font-size: 1.1rem; font-weight: 600; color: #ffffff; margin-bottom: 0.5rem;">${escapeHtml(item.briefTitleEn || item.titleEn)}</h3>
            ${item.kickerEn ? `<p style="font-size: 0.75rem; color: #6b6b6b; margin-bottom: 0.5rem;">${escapeHtml(item.kickerEn)}</p>` : ''}
            ${item.summaryEn ? `<p style="font-size: 0.875rem; color: #888888; line-height: 1.6;">${escapeHtml(item.summaryEn)}</p>` : ''}
          </div>
        `).join('\n')
      : '<p style="color: #888888;">No briefing today.</p>';

    const bodyHtml = `
      <div class="site-container fade-in" style="padding-top: 3rem; padding-bottom: 6rem;">
        <a href="/blog" style="font-size: 0.8125rem; color: #6b6b6b; display: inline-flex; align-items: center; gap: 0.25rem; margin-bottom: 2.5rem; transition: color 0.15s;">← AI Briefing</a>
        <header style="margin-bottom: 2.5rem;">
          <span class="tag-pill" style="margin-bottom: 0.75rem; display: inline-block;">AI Daily</span>
          <h1 style="font-size: 1.625rem; font-weight: 600; color: #ffffff; line-height: 1.3; letter-spacing: -0.025em; margin-bottom: 0.75rem; margin-top: 0.5rem;">${escapeHtml(digest.titleEn)}</h1>
          <div style="display: flex; gap: 1rem; align-items: center; flex-wrap: wrap;">
            <span style="font-size: 0.8rem; color: #6b6b6b;">${formatDisplayDate(digest.date)}</span>
            ${digest.itemCount ? `<span style="font-size: 0.8rem; color: #6b6b6b;">${digest.itemCount} items</span>` : ''}
          </div>
          ${digest.excerptEn ? `<p style="font-size: 0.9375rem; color: #888888; line-height: 1.65; margin-top: 1rem; border-left: 2px solid #1f1f1f; padding-left: 1rem;">${escapeHtml(digest.excerptEn)}</p>` : ''}
        </header>
        <hr style="border: none; border-top: 1px solid #1f1f1f; margin-bottom: 2.5rem;" />
        <div class="prose-blog">${digestBodyHtml}</div>
        <div style="margin-top: 4rem; padding-top: 2rem; border-top: 1px solid #1f1f1f;">
          <div style="display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 1rem;">
            <a href="/blog" style="font-size: 0.8125rem; color: #6b6b6b;">← Back to briefing</a>
            <a href="https://x.com/intent/tweet?text=${encodeURIComponent(digest.titleEn)}&url=${encodeURIComponent(routeUrl(`blog/${digest.slug}`))}&via=GuangtaoS29545" target="_blank" rel="noopener noreferrer" class="x-cta">
              <svg width="12" height="12" viewBox="0 0 24 24" fill="currentColor"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-4.714-6.231-5.401 6.231H2.744l7.73-8.835L1.254 2.25H8.08l4.253 5.622zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg>
              Share on X
            </a>
          </div>
    ...
    `;
    writeRouteHtml(template, `blog/${digest.slug}`, digestRoute, bodyHtml);
  }

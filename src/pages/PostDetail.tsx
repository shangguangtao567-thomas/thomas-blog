  ...
  return (
    <div className="min-h-screen">
      <article className="max-w-2xl mx-auto px-6 md:px-8 pt-10 pb-20">
        ...
        <h1 className="text-3xl md:text-4xl font-display text-foreground leading-tight mb-4">{title}</h1>
        <p className="text-base md:text-lg text-muted-foreground font-ui leading-relaxed mb-6">{excerpt}</p>
        <GrowthActions language={language} context={`post_${post.slug}`} className="mb-8" compact />
              ...
              {relatedPosts.map(item => (
                ...
                >
                  ...
                  <p className="text-sm text-muted-foreground font-ui leading-relaxed">
                    {truncateText(language === 'zh' ? item.excerptZh : item.excerptEn, 150)}
                  </p>
                </button>
              ))}
            </div>

...
export default function Home({ navigate }: HomeProps) {
  ...
  const posts = allPosts.filter(post => !post.slug.startsWith('ai-daily-')).slice(0, 4);
  const articleCount = allPosts.filter(post => !post.slug.startsWith('ai-daily-')).length;
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_1.6fr] gap-10 mb-14">
          <div className="flex flex-col gap-6">
            ...
            <div className="grid grid-cols-3 gap-3 pt-1">
              {[
                { value: String(articleCount), label: language === 'zh' ? '篇文章' : 'Posts' },
                { value: String(digests.length), label: language === 'zh' ? '期日报' : 'Digests' },
        ...
        <div className="border-t border-border pt-10 space-y-5">
          ...
          {latestDigest && (
            <button
              ...
              className="w-full text-left rounded-[24px] border border-border bg-card p-6 hover:border-foreground/20 hover:shadow-sm transition-all"
            >
              ...
              <p className="text-sm text-muted-foreground font-ui leading-relaxed mb-4 max-w-3xl">
                {truncateText(language === 'zh' ? (latestDigest.issueSummaryZh || latestDigest.excerptZh) : (latestDigest.issueSummaryEn || latestDigest.excerptEn), 240)}
              </p>
            ...
            {techNews.map(item => (
              <div key={item.id} className="group flex flex-col gap-2 p-4 rounded-xl bg-card border border-border">
                <div className="flex items-start justify-between gap-2">
                  ...
                  <span className="text-[11px] text-muted-foreground font-mono">{language === 'zh' ? item.publishedLabelZh || item.source : item.publishedLabelEn || item.sourceName || item.source}</span>
                </div>
                ...
                <p className="text-xs text-muted-foreground font-ui line-clamp-4">
                  {truncateText(language === 'zh' ? item.deckZh || item.summaryZh : item.deckEn || item.summaryEn, 140)}
                </p>
              </div>
            ))}
          </div>
